...
QString MainWindow::getArchOption() const
{
    static const QMap<QString, QString> archMap {
        {"x86_64", "--arch=x86_64"}, {"i386", "--arch=i386"}, {"arm", "--arch=arm"}, {"arm64", "--arch=aarch64"}};
...
QStringList MainWindow::listFlatpaks(const QString &remote, const QString &type) const
{
    ...
    const QString archFp = getArchOption().trimmed();
    ...
    auto runRemoteLs = [&typeFlag](QStringList args) {
        ...
        qDebug() << "Running Flatpak remote listing:" << args;
        Cmd command;
        QString output;
    ...
    if (list.isEmpty()) {
        ...
        const QString updateScope = fpUser.trimmed();
        if (!updateScope.isEmpty()) {
            updateArgs << updateScope;
        }
...
void MainWindow::pushUninstall_clicked()
{
    ...
    if (currentTab == Tab::Flatpak) {
        ...
        for (const QString &app : std::as_const(changeList)) {
            enableOutput();
            QStringList args {"flatpak", "uninstall"};
            args << fpUser.trimmed();
            args << "-y" << app;
            if (!cmd.run(flatpakPtyCommand(shellCommandFromArgs(args)))) { // success if all processed successfuly,
...
void MainWindow::setupSnapd()
{
    ...
    if (ready && !coreInstalled) {
        ...
        for (int attempt = 1; attempt <= maxAttempts && !coreInstalled; ++attempt) {
            ...
            // Route through the MXPI helper (auth_admin_keep) so the cached elevation is
            // reused instead of prompting again.
