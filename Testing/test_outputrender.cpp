...
private slots:
    ...
    {
        // The real-world case: snap redraws its progress line ~10x/second and each redraw
        // is delivered as its own chunk. They must collapse to one line, not pile up.
        QStringList chunks;
    ...
    void cursorStyleSequenceIgnored()
    {
        // CSI with an intermediate byte, e.g. cursor-style "ESC [ 2 SP q", must be
        // consumed whole, not leave a stray "q".
        QCOMPARE(render({"\x1B[2 qworking\r\n"}), QString("working\n"));
    }
